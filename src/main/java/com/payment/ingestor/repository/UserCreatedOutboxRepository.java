...
@Repository
public interface UserCreatedOutboxRepository extends JpaRepository<UserCreatedOutbox, UUID> {

    @Query(
            value = """
                    SELECT *
                    FROM users_created_outbox
    ...
    List<UserCreatedOutbox> findPendingForUpdate(@Param("batchSize") int batchSize);

    @Modifying
    @Query(
            value = """
                    UPDATE users_created_outbox
